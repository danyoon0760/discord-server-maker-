...
type FormState = Omit<TemplateItem, "id" | "tags"> & {
  tags: string;
};
...
export default function TemplatePage() {
  ...
  function saveTemplate() {
    ...
    const nextItem: TemplateItem = {
      ...
      members: form.members.trim() || "서버 템플릿",
      ...
      <section className="mx-auto max-w-6xl">
        ...
        <div className="mt-8 rounded-[2rem] border border-white/10 bg-zinc-950/70 p-8 shadow-2xl">
          ...
          <p className="mt-4 max-w-2xl text-zinc-400">
            디스보드처럼 카드로 템플릿을 정리합니다. 지금은 브라우저 안에서
            추가·수정·삭제되는 테스트 버전입니다.
          </p>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[360px_1fr]">
          <div className="rounded-2xl border border-white/10 bg-zinc-950/70 p-5">
            ...
            <p className="mt-2 text-sm text-zinc-500">
              링크에는 실제 디스코드 서버 템플릿 URL을 넣으면 됩니다.
            </p>
            ...
            <div className="mt-5 grid gap-3">
              ...
              <input className="rounded-xl border border-white/10 bg-black/40 px-4 py-3" placeholder="태그: 발로란트, 게임, 내전" value={form.tags} onChange={(event) => updateForm("tags", event.target.value)} />
              <input className="rounded-xl border border-white/10 bg-black/40 px-4 py-3" placeholder="분류: 게임 서버" value={form.members} onChange={(event) => updateForm("members", event.target.value)} />
              ...
              <article key={template.id} className="overflow-hidden rounded-2xl border border-white/10 bg-zinc-950/70">
                <div className="border-b border-white/10 bg-white/[0.03] p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="text-sm text-emerald-300">● {template.members}</p>
