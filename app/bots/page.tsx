...
const botTags = [
  ...
  "유틸리티", "빗금 명령어", "뮤직", "대시보드", "번역", "경제", "해외봇", "서버관리", "알림", "방송", "음성",
];
...
const fallbackBots: BotItem[] = [
  ...
  { id: -2, name: "DISBOARD", description: "디스코드 서버 홍보와 검색에 많이 쓰이는 대표적인 서버 리스트 봇입니다.", tags: ["홍보", "서버관리"], category: "홍보", link: "https://disboard.org/" },
...
export default function BotsPage() {
  ...
  const loadBots = useCallback(async () => {
    setIsLoading(true);
    if (!canUseSupabase) {
      setMessage("Supabase 환경변수를 넣으면 DB 목록을 사용합니다.");
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : "봇 목록을 불러오지 못했습니다.");
      setBots([]);
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    ...
    if (!botName) return;
    const matchedBot = bots.find((bot) => bot.name.trim().toLowerCase() === botName.trim().toLowerCase());
  ...
  function updateInlineForm(bot: BotItem, field: keyof BotFormState, value: string | string[]) {
    ...
    setInlineForms((prev) => ({
      ...prev,
      [key]: {
        name: prev[key]?.name ?? bot.name,
        description: prev[key]?.description ?? bot.description,
        tags: prev[key]?.tags ?? bot.tags,
        link: prev[key]?.link ?? bot.link,
        [field]: value,
      },
    }));
  }
  ...
  function toggleTag(tag: string) {
    setForm((prev) => ({ ...prev, tags: prev.tags.includes(tag) ? prev.tags.filter((item) => item !== tag) : [...prev.tags, tag] }));
  }

  function toggleInlineTag(bot: BotItem, tag: string) {
    const key = makeKey(bot);
    const currentTags = inlineForms[key]?.tags ?? bot.tags;
    updateInlineForm(bot, "tags", currentTags.includes(tag) ? currentTags.filter((item) => item !== tag) : [...currentTags, tag]);
  }
  ...
  function makePayload(currentForm: BotFormState, fallbackCategory?: string) {
    const tags = uniqueTags(currentForm.tags);
    return {
      name: currentForm.name.trim(),
      description: currentForm.description.trim() || "설명이 아직 없습니다.",
      tags,
      category: tags[0] || fallbackCategory || "기타",
      link: currentForm.link.trim() || "https://discord.com",
    };
  }

  async function saveBot() {
    if (!adminPassword) return alert("관리자 로그인이 필요합니다.");
    if (!form.name.trim()) return alert("봇 이름을 입력해주세요.");
    setSavingKey("new");
    try {
      await adminRequest("/api/admin/bots", adminPassword, { method: "POST", body: JSON.stringify(makePayload(form)) });
      setForm(emptyForm);
      setIsFormOpen(false);
      await loadBots();
    } catch (error) {
  ...
  async function saveInlineBot(bot: BotItem) {
    if (!adminPassword) return alert("관리자 로그인이 필요합니다.");
    const key = makeKey(bot);
    const currentForm = inlineForms[key] ?? { name: bot.name, description: bot.description, tags: bot.tags, link: bot.link };
    if (!currentForm.name.trim()) return alert("봇 이름을 입력해주세요.");
    setSavingKey(key);
    try {
      if (bot.id > 0) await adminRequest("/api/admin/bots", adminPassword, { method: "PATCH", body: JSON.stringify({ id: bot.id, ...makePayload(currentForm, bot.category) }) });
      else await adminRequest("/api/admin/bots", adminPassword, { method: "POST", body: JSON.stringify(makePayload(currentForm, bot.category)) });
      await loadBots();
    } catch (error) {
      alert(error instanceof Error ? error.message : "저장에 실패했습니다.");
    } finally {
  ...
  async function deleteBot(bot: BotItem) {
    ...
    try {
      await adminRequest(`/api/admin/bots?id=${bot.id}`, adminPassword, { method: "DELETE" });
      setBots((prev) => prev.filter((item) => item.id !== bot.id));
      setInlineForms((prev) => {
        const next = { ...prev };
        delete next[makeKey(bot)];
        return next;
      });
      if (selectedBot?.id === bot.id) closeSelectedBot();
    } catch (error) {
      ...
      <section className="mx-auto max-w-6xl">
        ...
        <div className="mt-8 rounded-[2rem] border border-white/10 bg-zinc-950/70 p-8 shadow-2xl">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            ...
            {isAdmin && <button onClick={() => setIsFormOpen(true)} className="rounded-2xl bg-indigo-500 px-5 py-3 text-sm font-bold text-white hover:bg-indigo-400">봇 추가</button>}
          </div>
          {message && <p className="mt-4 rounded-2xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-200">{message}</p>}
              ...
              <article key={key} className="flex h-[390px] flex-col overflow-hidden rounded-2xl border border-white/10 bg-zinc-950/70">
                ...
                <div className="flex flex-1 flex-col justify-between p-5 text-sm text-zinc-300">
                  {isAdmin ? (
                    <div className="space-y-3">
                      ...
                      <div className="flex max-h-20 flex-wrap gap-2 overflow-y-auto rounded-lg border border-white/10 bg-black/20 p-2">
                        {botTags.map((tag) => <button key={tag} type="button" onClick={() => toggleInlineTag(bot, tag)} className={`rounded-md px-2 py-1 text-[11px] font-semibold ${inlineForm.tags.includes(tag) ? "bg-indigo-500 text-white" : "bg-white/5 text-zinc-400 hover:bg-white/10"}`}>{tag}</button>)}
                      </div>
                    </div>
                  ) : (
                    <p className="line-clamp-3 min-h-[84px] leading-7 text-zinc-300">{bot.description}</p>
                  )}
                  <div className="flex flex-wrap gap-2 pt-4">
                    ...
                    {isAdmin && <button disabled={savingKey === key} onClick={() => saveInlineBot(bot)} className="rounded-lg border border-indigo-400/40 px-3 py-2 text-xs text-indigo-200 hover:bg-indigo-500/10 disabled:cursor-not-allowed disabled:opacity-60">{savingKey === key ? "저장 중" : "저장"}</button>}
                    {isAdmin && <button onClick={() => deleteBot(bot)} className="rounded-lg border border-red-500/30 px-3 py-2 text-xs text-red-300 hover:bg-red-500/10">삭제</button>}
                  </div>
                </div>
              </article>
            );
      ...
      {isFormOpen && isAdmin && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 backdrop-blur-sm">
          <div className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-3xl border border-white/10 bg-[#0b0c12] p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4"><div><p className="text-sm font-semibold text-indigo-300">BOT FORM</p><h2 className="mt-2 text-3xl font-black">봇 추가</h2></div><button onClick={() => setIsFormOpen(false)} className="rounded-xl border border-white/10 px-4 py-2 text-sm text-zinc-300 hover:bg-white/5">닫기</button></div>
            ...
            <button disabled={savingKey === "new"} onClick={saveBot} className="mt-5 w-full rounded-xl bg-indigo-500 px-4 py-3 font-semibold hover:bg-indigo-400 disabled:cursor-not-allowed disabled:opacity-60">{savingKey === "new" ? "저장 중" : "추가"}</button>
          </div>
        </div>
      )}

      {selectedBot && (
        <div onMouseDown={(event) => setIsBackdropPressed(event.target === event.currentTarget)} onMouseUp={(event) => { if (isBackdropPressed && event.target === event.currentTarget) closeSelectedBot(); setIsBackdropPressed(false); }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 backdrop-blur-sm">
